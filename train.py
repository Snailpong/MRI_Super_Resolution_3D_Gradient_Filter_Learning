...
#
#     quantization, patchNumber = quantization_border(im_LR, C.G_WEIGHT, patchNumber, quantization)  # get the strength and coherence of each patch
...
for file_idx, file in enumerate(file_list):
    ...
    im_HR, im_LR = get_train_image(file)
    Q, V = train_qv(im_LR, im_HR, stre, cohe, Q, V)  # get Q, V of each patch

    if file_idx + 1 == 10:
        break
    
    print(' ' * 26, 'last', '%.1f' % ((time.time() - file_timer) / 60), 'min', end='', flush=True)
